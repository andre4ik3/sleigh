...
async def test_valid(cli):
	for i in range(0, 1000):
		...
		pf_resp = await cli.post(f"/preflight/{preflight._data['machine_id']}", data=dumps(preflight._data))
		...
		path = Path(f"config/events/{year}/{month}/{day}/{preflight._data['machine_id']}.json").resolve()
